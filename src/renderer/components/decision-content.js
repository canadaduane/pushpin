...
export default class DecisionContent extends React.PureComponent {
  ...
  handleClickThumbsUp = proposalId => e => {
    const isMyNomination = Object.keys(this.state.proposals[proposalId].nominations).includes(
      this.props.selfId
    )
    if (isMyNomination) {
      this.handle.change(doc => {
  ...
  notionalProposals = () =>
    Object.entries(this.state.proposals).filter(
      proposal => Object.keys(proposal[1].nominations).length <= 1
    )

  musteredProposals = () =>
    Object.entries(this.state.proposals).filter(
      proposal => Object.keys(proposal[1].nominations).length >= 2
    )
  ...
  renderVoteMode = () => (
    <React.Fragment>
      ...
      {this.musteredProposals().map(arrProp => {
        ...
        const proposal = arrProp[1]
        return (
          <div style={css.proposalVote} key={proposalId} onDrag={this.stopPropagation}>
